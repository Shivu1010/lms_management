...
const ExtraSideData = () => {
  ...
  return <>
      {!isDesktop && <div className="asidebar-button d-xl-none d-flex">
          <button className="btn btn-danger btn-icon rounded-end-0" onClick={toggle}>
            <TbAdjustmentsHorizontal className="fs-xl" />
          </button>
        </div>}

      {!isDesktop ? <Offcanvas show={show} onHide={toggle} placement="end" className={`asidebar border-start offcanvas-xl overflow-hidden`} tabIndex={-1}>
          <SimplebarClient className="offcanvas-body p-0 h-100">
            <SidebarContent />
          </SimplebarClient>
        </Offcanvas> : <div className="asidebar border-start">
          <div className="h-100">
            <SidebarContent />
          </div>
        </div>}
